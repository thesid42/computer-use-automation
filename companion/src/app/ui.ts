...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .request-layout { display: grid; grid-template-columns: minmax(0, 1.45fr) minmax(250px, .75fr); gap: 16px; }
    .request-composer { padding: 22px; }
    .request-kicker { margin-bottom: 18px; color: var(--accent-dark); font-size: 10px; font-weight: 800; letter-spacing: .1em; text-transform: uppercase; }
    .request-composer > label { display: block; margin-bottom: 8px; color: var(--ink); font-size: 17px; font-weight: 760; letter-spacing: -.02em; }
    .request-goal { min-height: 136px; font-size: 15px; }
    .request-composer .field-hint { display: block; max-width: 680px; margin-top: 9px; }
    .request-actions { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-top: 19px; padding-top: 15px; border-top: 1px solid var(--line); }
    .request-guide { align-self: start; padding: 19px; background: #fbfaf9; }
    .request-guide h3, .request-activity h3 { margin: 0; font-size: 14px; letter-spacing: -.015em; }
    .request-guide ol { display: grid; gap: 14px; margin: 17px 0 0; padding: 0; list-style: none; }
    .request-guide li { display: grid; grid-template-columns: 24px minmax(0, 1fr); gap: 10px; align-items: start; }
    .request-guide li > span { display: grid; width: 24px; height: 24px; place-items: center; color: var(--accent-dark); font-size: 10px; font-weight: 800; border: 1px solid #d1cae8; border-radius: 50%; background: var(--accent-wash); }
    .request-guide strong { display: block; color: var(--ink); font-size: 11px; }
    .request-guide small { display: block; margin-top: 3px; color: var(--muted); font-size: 10px; line-height: 1.4; }
    .guide-examples { display: grid; gap: 6px; margin-top: 19px; padding-top: 14px; border-top: 1px solid var(--line); }
    .guide-examples .button { justify-content: flex-start; text-align: left; }
    .request-activity { margin-top: 16px; overflow: hidden; }
    .request-activity-head { display: flex; align-items: flex-end; justify-content: space-between; gap: 12px; padding: 17px 19px 13px; border-bottom: 1px solid var(--line); }
    .request-activity-head p { margin: 5px 0 0; color: var(--muted); font-size: 11px; }
    .request-activity-list { min-height: 40px; }
    .request-activity-row { display: grid; grid-template-columns: minmax(0, 1fr) 120px auto; gap: 14px; align-items: center; padding: 13px 19px; border-bottom: 1px solid #ecebed; }
    .request-activity-row:last-child { border-bottom: 0; }
    .request-activity-goal { overflow: hidden; color: var(--ink); font-size: 12px; font-weight: 700; text-overflow: ellipsis; white-space: nowrap; }
    .request-activity-meta { margin-top: 4px; color: var(--faint); font-size: 10px; }
    ...
    @media (max-width: 620px) {
      ...
      .request-composer { padding: 17px; }
      .request-actions { align-items: stretch; flex-direction: column; }
      .request-actions .button { width: 100%; }
      .request-activity-head { align-items: flex-start; flex-direction: column; }
      .request-activity-head .button { width: 100%; }
      .request-activity-row { grid-template-columns: minmax(0, 1fr) auto; gap: 7px 12px; }
      .request-activity-row .status-badge { grid-column: 2; grid-row: 1 / span 2; }
      .request-activity-row .button { grid-column: 1; }
...
<body>
  <div class="app-shell">
    <aside class="sidebar" aria-label="Primary navigation">
      ...
      <nav class="nav">
        <button class="nav-button" data-view="request" aria-current="page" title="New request"><span class="nav-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M5 5h14v11H9l-4 4V5Z"/><path d="M8 9h8M8 12h5"/></svg></span><span>New request</span></button>
        <button class="nav-button" data-view="workflows" title="Automations"><span class="nav-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><rect x="4" y="4" width="6" height="6" rx="1"/><rect x="14" y="4" width="6" height="6" rx="1"/><rect x="4" y="14" width="6" height="6" rx="1"/><path d="M14 17h6M17 14v6"/></svg></span><span>Automations</span></button>
    ...
    <main class="main">
      <header class="topbar"><div><nav class="breadcrumbs" id="breadcrumbs" aria-label="Breadcrumb"><a href="#request">New request</a></nav><h1 id="view-title">New request</h1></div><div class="target-chip"><span class="connection-dot" id="top-connection-dot" aria-hidden="true"></span><span id="top-target">Configured target</span></div></header>
      <div class="content">
        <section class="view is-active" data-panel="request" aria-labelledby="view-title">
          <div class="section-header"><div><h2>Ask the companion</h2><p>Describe the outcome you need in your own words. The companion can learn the path once and make it reusable.</p></div><button class="button" id="new-automation-button" type="button"><span aria-hidden="true">＋</span> Teach a task</button></div>
          <div class="request-layout"><form class="panel request-composer" id="task-form"><div class="request-kicker">Member Servicing · configured target</div><label for="request-goal">What would you like to do?</label><textarea class="text-area request-goal" id="request-goal" name="goal" required autocomplete="off" placeholder="Check a member's savings balance, find recent transactions, or request a payoff quote."></textarea><p class="field-hint">Include the member or account details and dates in your request when you have them. The companion will ask a follow-up when it needs more information.</p><div class="request-actions"><span class="privacy-note">Request details stay in this run.</span><button class="button primary" id="request-run-button" type="submit">Start request</button></div><div id="request-feedback" aria-live="polite"></div></form><aside class="request-guide panel"><h3>How it works</h3><ol><li><span>1</span><div><strong>Describe the outcome</strong><small>Use ordinary language, with the details you know.</small></div></li><li><span>2</span><div><strong>Watch the browser</strong><small>The companion learns or replays the configured screens.</small></div></li><li><span>3</span><div><strong>Reuse the result</strong><small>Successful paths appear in Automations for the next request.</small></div></li></ol><div class="guide-examples"><div class="field-hint">Examples to adapt</div><button class="button small" type="button" data-request-example="Check the savings balance for member 12345.">Savings balance</button><button class="button small" type="button" data-request-example="Find transactions for member 12345 from 2026-09-01 to 2026-09-11.">Transactions in a date range</button><button class="button small" type="button" data-request-example="Get the loan payoff quote for member 12345 as of 2026-09-30.">Loan payoff quote</button></div></aside></div><section class="panel request-activity"><div class="request-activity-head"><div><h3>Recent requests</h3><p>Follow a request while it learns, runs, or waits for your answer.</p></div><a class="button small" href="#history">View history</a></div><div id="request-activity-list"><div class="empty-subcopy">No requests yet.</div></div></section>
        </section>
        <section class="view" data-panel="workflows" aria-labelledby="view-title">
          <div class="section-header"><div><h2>Automation library</h2><p>Save a workflow once. Run it again with a fresh request.</p></div><button class="button primary" id="new-automation-library-button" type="button"><span aria-hidden="true">＋</span> New automation</button></div>
  ...
  <dialog class="dialog" id="new-automation-dialog" aria-labelledby="new-automation-title"><div class="dialog-head"><div><h2 id="new-automation-title">New automation</h2><p>Teach the companion a repeatable task in plain language.</p></div><button class="icon-button" id="close-new-automation" type="button" aria-label="Close new automation dialog">×</button></div><form class="dialog-body" id="dialog-task-form"><div class="scope-note">The companion learns the supported member servicing screens from your request. It currently handles savings balances, transaction date ranges, and loan payoff quotes; describe the outcome plainly and include the details you have.</div><div class="field"><label for="goal">What should the automation do?</label><textarea class="text-area" id="goal" name="goal" required autocomplete="off" placeholder="Look up member 12345 and tell me their savings balance."></textarea><span class="field-hint">A successful run becomes a reusable automation. Any values you mention are used for this run only.</span></div><div class="example-row" style="display:grid;gap:6px;margin-top:13px"><span class="field-hint">Try an example</span><button class="button small" type="button" data-example-goal="Look up member 12345 and tell me their savings balance.">Savings balance for member 12345</button><button class="button small" type="button" data-example-goal="Find transactions for member 12345 from 2026-09-01 to 2026-09-11.">Transactions · Sep 1–11, 2026</button><button class="button small" type="button" data-example-goal="Get the loan payoff quote for member 12345 as of 2026-09-30.">Loan payoff quote · Sep 30, 2026</button></div><div id="dialog-task-feedback" aria-live="polite"></div><div class="dialog-actions"><button class="button" id="cancel-new-automation" type="button">Cancel</button><button class="button primary" id="run-button" type="submit">Learn automation</button></div></form></dialog>
  ...
  (() => {
    ...
    const state = { activeView: 'request', runs: [], workflows: [], context: null, selectedWorkflowId: '', selectedRunId: '', pollTimers: new Map(), routeToken: 0, submitBusy: false, editWorkflowId: '', lastFocus: null, conversationContext: null, conversationId: '' };
    ...
    function renderRequestActivity() { const host = $('#request-activity-list'); if (!host) return; if (!state.runs.length) { host.innerHTML = '<div class="empty-state"><strong>No requests yet</strong><span class="empty-subcopy">Start with a plain-language request above. Progress and results will appear here.</span></div>'; return; } host.innerHTML = state.runs.slice(0, 4).map((run) => '<div class="request-activity-row"><div><div class="request-activity-goal">' + escapeHtml(run.goal || runName(run)) + '</div><div class="request-activity-meta">' + escapeHtml(runName(run)) + ' · ' + escapeHtml(formatDate(run.createdAt || run.updatedAt)) + '</div></div><span class="status-badge ' + escapeHtml(runStatus(run)) + '">' + escapeHtml(statusLabel(runStatus(run))) + '</span><a class="button small" href="#runs/' + encodeURIComponent(run.id) + '" data-run-id="' + escapeHtml(run.id) + '">Open request</a></div>').join(''); bindRunLinks(host); }
    ...
    function runFormHtml(workflow) { const description = workflowDescription(workflow); const example = description.includes('transactions') ? 'Find transactions for a member between two dates.' : description.includes('payoff') ? 'Get the payoff quote for a member on a chosen date.' : 'Check the current savings balance for a member.'; return '<div class="field"><label for="workflow-run-goal">Describe this run</label><textarea class="text-area" id="workflow-run-goal" name="goal" required autocomplete="off" placeholder="' + escapeHtml(example) + '"></textarea><span class="field-hint">Use a fresh request each time. The saved path stays unchanged.</span></div>'; }
    ...
    async function submitWorkflowRun(event, workflow) { event.preventDefault(); if (state.submitBusy) return; const form = event.currentTarget; const button = $('#run-workflow-button', form); const feedback = $('#run-submit-feedback', form); const goal = ($('#workflow-run-goal', form)?.value || '').trim(); if (!goal || !form.reportValidity()) return; state.submitBusy = true; button.disabled = true; button.innerHTML = '<span class="spinner" aria-hidden="true"></span>Starting'; setLoading(feedback, 'Starting run'); try { const id = workflowId(workflow); const selectedContext = { workflowId: id, source: 'saved_automation' }; const created = await api('/api/tasks', { method: 'POST', headers: { 'content-type': 'application/json', 'Prefer': 'respond-async' }, body: JSON.stringify({ goal, context: selectedContext }) }); const runId = created.runId || created.id || created.run?.id; if (!runId) throw new Error('The server did not return a run id.'); form.reset(); showToast('Run started.'); window.location.hash = '#runs/' + encodeURIComponent(runId); } catch (error) { setError(feedback, error.message); showToast(error.message); } finally { state.submitBusy = false; button.disabled = workflowStatus(workflow) === 'archived'; button.textContent = 'Run automation'; } }
    ...
    $('#new-automation-library-button').addEventListener('click', openNewDialog);
    ...
    $('#task-form').addEventListener('submit', submitRequest);
    $('#dialog-task-form').addEventListener('submit', submitTask);
    ...
    $$('[data-request-example]').forEach((button) => button.addEventListener('click', () => { $('#request-goal').value = button.dataset.requestExample || ''; $('#request-goal').focus(); }));
    function addExtendedExamples() { const examples = [{ label: 'Service requests', goal: 'Show open service requests for member 12345.' }, { label: 'Branch directory', goal: 'Find the branch directory and show Northside hours.' }]; const hosts = [$('.guide-examples'), $('.example-row', $('#new-automation-dialog'))].filter(Boolean); hosts.forEach((host) => examples.forEach((example) => { const button = document.createElement('button'); button.type = 'button'; button.className = 'button small'; button.textContent = example.label; button.dataset.exampleGoal = example.goal; button.addEventListener('click', () => { const dialog = host.closest('dialog'); const field = dialog ? $('#goal') : $('#request-goal'); field.value = example.goal; field.focus(); }); host.appendChild(button); })); }
    addExtendedExamples();
    ...
    // Keep the operator-facing detail page focused on the readable flow and
    // free-text request. The raw contract remains available on demand.
    function detailWorkflowHtml(workflow) { const id = workflowId(workflow); const status = workflowStatus(workflow); const actions = workflowActions(workflow); const inputs = workflowInputs(workflow); const outputs = workflowOutputs(workflow); const checkpoint = workflow.finalCheckpoint || workflow.checkpoint || (Array.isArray(workflow.postconditions) ? workflow.postconditions[0] : '') || 'The final result is verified by the replay runner.'; const steps = actions.length ? actions.map((action, index) => '<li class="step"><span class="step-marker" aria-hidden="true">' + (index + 1) + '</span><div class="step-body"><div class="step-title">' + escapeHtml(actionDescription(action)) + '<span class="step-kind">' + escapeHtml(actionKindLabel(action.kind)) + '</span></div></div></li>').join('') : '<li class="empty-subcopy">Step details are not available for this automation.</li>'; const rawContract = { inputs, outputs, preconditions: workflow.preconditions || [], postconditions: workflow.postconditions || [], finalCheckpoint: checkpoint }; return '<a class="detail-back" href="#automations"><svg viewBox="0 0 24 24"><path d="m15 18-6-6 6 6"/></svg>Back to automations</a><article class="panel"><div class="detail-head"><div><h2 id="workflow-detail-title">' + escapeHtml(workflowName(workflow)) + '</h2><p class="detail-description">' + escapeHtml(workflowDescription(workflow)) + '</p><div class="detail-meta"><span class="status-badge ' + escapeHtml(status) + '">' + escapeHtml(statusLabel(status)) + '</span><span>Version ' + escapeHtml(workflowVersion(workflow)) + '</span><span>' + escapeHtml(actions.length || 0) + ' steps</span></div></div><div class="detail-actions">' + (status === 'archived' ? '<button class="button" data-workflow-action="restore" data-workflow-id="' + escapeHtml(id) + '" type="button">Restore</button>' : '<button class="button" data-workflow-action="archive" data-workflow-id="' + escapeHtml(id) + '" type="button">Archive</button>') + '<button class="button" data-workflow-action="edit" data-workflow-id="' + escapeHtml(id) + '" type="button">Edit details</button></div></div><div class="detail-grid"><div><section class="subsection"><h3>Workflow steps</h3><p class="subsection-intro">The saved path runs in this order and checks each checkpoint.</p><ol class="steps">' + steps + '</ol></section><section class="subsection"><h3>Completion check</h3><p class="subsection-intro">The run is complete only after this condition is verified.</p><div class="schema-row"><div class="schema-key">Final condition</div><div class="schema-value">' + escapeHtml(checkpoint) + '</div></div></section><details class="contract-details"><summary>Automation details</summary><p class="subsection-intro">Input fields, result fields, and the checkpoint used for this automation.</p><h3>Inputs it needs</h3>' + schemaRows(inputs, 'inputs') + '<h3>What it returns</h3>' + schemaRows(outputs, 'outputs') + '<pre class="contract-code">' + escapeHtml(safeJson(rawContract)) + '</pre></details></div><form class="run-form-panel" id="run-workflow-form" data-workflow-id="' + escapeHtml(id) + '"><h3>Run this automation</h3><p>Describe a fresh request in plain language. The saved workflow stays reusable.</p>' + runFormHtml(workflow) + '<div class="run-form-actions"><span class="privacy-note">Request details are not saved in browser storage.</span><button class="button primary" id="run-workflow-button" type="submit"' + (status === 'archived' ? ' disabled' : '') + '>Run automation</button></div><div id="run-submit-feedback" aria-live="polite"></div></form></div></article>'; }
    function taskRunId(payload) { return payload?.runId || payload?.id || payload?.run?.id || payload?.task?.id || ''; }
    function taskQuestion(payload) { return payload?.question || payload?.message || payload?.clarification?.question || payload?.needsInput?.question || payload?.needs_input?.question || payload?.result?.question || ''; }
    function showTaskClarification(form, question, continueTask) { const feedback = $('[aria-live="polite"]', form); if (!feedback) return; feedback.innerHTML = '<div class="clarification"><strong>A little more detail is needed</strong><p>' + escapeHtml(question || 'What detail should the companion use to continue?') + '</p><label for="task-clarification-answer">Your answer</label><textarea class="text-area" id="task-clarification-answer" required></textarea><button class="button primary" id="continue-task-button" type="button">Continue request</button></div>'; $('#task-clarification-answer', feedback)?.focus(); $('#continue-task-button', feedback)?.addEventListener('click', () => { const answer = ($('#task-clarification-answer', feedback)?.value || '').trim(); if (!answer) return; continueTask(answer); }); }
    function clearTaskComposer(form, feedback) { form.reset(); if (feedback) feedback.innerHTML = ''; state.conversationId = ''; state.conversationContext = null; }
    async function startTextTask(form, input, button, feedback) { const goal = (input?.value || '').trim(); if (!goal || state.submitBusy) return; state.submitBusy = true; button.disabled = true; button.innerHTML = '<span class="spinner" aria-hidden="true"></span>Starting'; if (feedback) setLoading(feedback, 'Starting request'); try { const context = state.conversationContext ? { ...state.conversationContext } : undefined; const body = { goal, ...(state.conversationId ? { conversationId: state.conversationId } : {}), ...(context ? { context } : {}) }; const created = await api('/api/tasks', { method: 'POST', headers: { 'content-type': 'application/json', 'Prefer': 'respond-async' }, body: JSON.stringify(body) }); const question = taskQuestion(created); const runId = taskRunId(created); if (question || created.status === 'needs_input' || created.status === 'needs-input') { state.conversationId = created.conversationId || created.conversation_id || state.conversationId; state.conversationContext = context || {}; showTaskClarification(form, question, (answer) => { input.value = answer; startTextTask(form, input, button, feedback); }); return; } state.conversationContext = null; state.conversationId = ''; if (runId) { clearTaskComposer(form, feedback); if (form.closest('dialog')) closeDialog(form.closest('dialog')); await loadRuns(); showToast('Request started.'); window.location.hash = '#runs/' + encodeURIComponent(runId); } else { if (feedback) feedback.innerHTML = '<div class="notice">Request accepted. Watch Recent requests for progress.</div>'; showToast('Request accepted.'); await loadRuns(); } } catch (error) { if (feedback) setError(feedback, error.message); showToast(error.message); } finally { state.submitBusy = false; button.disabled = false; button.textContent = form.closest('dialog') ? 'Learn automation' : 'Start request'; } }
    async function submitRequest(event) { event.preventDefault(); const form = event.currentTarget; await startTextTask(form, $('#request-goal', form), $('#request-run-button', form), $('#request-feedback', form)); }
    async function submitTask(event) { event.preventDefault(); const form = event.currentTarget; await startTextTask(form, $('#goal', form), $('#run-button', form), $('#dialog-task-feedback', form)); }
    function viewMeta(view) { return { request: ['New request', 'New request'], workflows: ['Automations', 'Automations'], attention: ['Needs attention', 'Needs attention'], history: ['Run history', 'Run history'], 'workflow-detail': ['Automations', 'Automation'], 'run-detail': ['Run history', 'Run details'] }[view] || ['New request', 'New request']; }
    async function route() { const token = ++state.routeToken; const raw = window.location.hash.replace(/^#/, '') || 'request'; const parts = raw.split('/').filter(Boolean).map((part) => decodeURIComponent(part)); let view = parts[0] === 'runs' ? 'run-detail' : parts[0] === 'automations' && parts[1] ? 'workflow-detail' : parts[0] === 'attention' ? 'attention' : parts[0] === 'history' ? 'history' : parts[0] === 'automations' ? 'workflows' : 'request'; state.activeView = view; $$('.view').forEach((panel) => panel.classList.toggle('is-active', panel.dataset.panel === view)); $$('.nav-button').forEach((button) => button.setAttribute('aria-current', button.dataset.view === view || (view === 'workflow-detail' && button.dataset.view === 'workflows') || (view === 'run-detail' && button.dataset.view === 'history') ? 'page' : 'false')); const meta = viewMeta(view); $('#view-title').textContent = view === 'workflow-detail' && parts[1] ? 'Automation' : meta[1]; const crumbs = $('#breadcrumbs'); const baseHref = view === 'history' || view === 'run-detail' ? '#history' : view === 'workflow-detail' ? '#automations' : '#request'; crumbs.innerHTML = '<a href="' + baseHref + '">' + escapeHtml(meta[0]) + '</a>' + (view === 'workflow-detail' || view === 'run-detail' ? '<span class="breadcrumb-sep">/</span><span>' + escapeHtml(meta[1]) + '</span>' : ''); if (view === 'request') { await loadRuns(); renderRequestActivity(); } else if (view === 'workflows') { await loadWorkflows(); await loadRuns(); } else if (view === 'history') { await loadRuns(); } else if (view === 'attention') { await loadRuns(); renderAttention(); } else if (view === 'workflow-detail') { if (!state.workflows.length) await loadWorkflows(); await loadWorkflowDetail(parts[1], token); } else if (view === 'run-detail') { if (!state.runs.length) await loadRuns(); await loadRunDetail(parts[1], token); } }
