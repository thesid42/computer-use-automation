...
type Mode = 'success' | 'not-found' | 'transient' | 'snapshot-transient';
class FakeSurface implements SurfaceAdapter {
  ...
  async observe(_session: SessionHandle): Promise<SurfaceSnapshot> { return { url: 'http://localhost:3001/servicing/member-search', title: 'Demo', framePath: [], controls: [], visibleText: this.transientVisible ? 'TEMPORARY_LOAD_FAILURE' : 'Current Balance', dialogs: [], stateFingerprint: this.transientVisible ? 'transient' : 'stable' }; }
  async resolve(_session: SessionHandle, _target: unknown): Promise<{ count: number; description: string }> { return this.transientVisible ? { count: 0, description: 'temporary page has no target' } : { count: 1, description: 'matched' }; }
  async act(_session: SessionHandle, action: { kind: string; condition?: string }): Promise<ActionResult> {
    ...
    if (action.kind === 'click' && this.mode === 'snapshot-transient' && !this.transientVisible) this.transientVisible = true;
...
const artifact = compileCapability(interpretGoal('Look up member 12345 and tell me their savings balance.'), []);
